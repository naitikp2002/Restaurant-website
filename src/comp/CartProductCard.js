  ...
  return (
    <Card sx={{ display: 'flex', margin:"1rem", maxWidth:"40rem",justifyContent: "space-between"}}>
      <Box sx={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <Button variant="outlined" onClick={()=>handleClikRemoveCart(props.props)}>-</Button>
        <Typography sx={{ textDecorationStyle: "solid"}} m={2}>✘ {props.count}</Typography>
        <Button variant="outlined" onClick={()=>handleClikAddCart(props.props)}>+</Button>
        </Box>
      </Box>
      {imageId && <CardMedia
        component="img"
